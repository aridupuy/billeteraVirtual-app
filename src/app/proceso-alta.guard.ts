    ...
    await this.proceso.validar().then(async (data: datosProceso) => {
      if(data.valida_ident==null || data.valida_ident ==undefined){
        const navigationExtras: NavigationExtras = {
          queryParams: {
            param: JSON.stringify({ login: true, url: "tabs/tabs/tab1"})
          }
        };
        this.navCtrl.navigateForward("identidad-pendiente", navigationExtras);
        resp = true
      }
        ...
        this.navCtrl.navigateForward("identidad-revalidar", navigationExtras);
        resp = true
      }
        ...
        this.navCtrl.navigateForward("revalida-mail", navigationExtras);
        resp = false
      }
        ...
        await this.validCel.reenviar_codigo().then(cel => {
          const navigationExtras: NavigationExtras = {
            queryParams: {
              param: JSON.stringify({ login: true, url: "tabs/tabs/tab1", cel: cel })
            }
          };
          this.navCtrl.navigateForward("registrar-paso2", navigationExtras);
